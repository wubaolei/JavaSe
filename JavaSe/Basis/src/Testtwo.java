...
public class Testtwo
{
	String name;				// 姓名 实例变量 
	int age;					// 年龄 实例变量
	static String cityname;		// 城市名字 静态变量
	
	
	public static void main(String[] args)			// 入口函数 main
	{
		...
		Testtwo b = new Testtwo("javascript",18);	// 结合下面 带参数的构造方法
...
*/	
	...
	{
		this.sage = sage;				// this表示当前对象，谁调用这个方法，this就表示谁
										// 但是  this 不能写在 static 静态方法中
	}
	
	public int getSage()				// get 获取值( 不同的类相互访问 )
	{
...
/*	
* 构造方法 	作用一：给类中的属性进行初始化赋值 	作用二：给类中创建对象提供访问机制
* 1.public可以不加   还可以换成 private 
* 2.构造函数 不能有返回值
* 3.构造方法的名称 和 类名保持一致
* 4.就是不能把数据类型写出来否则就不是构造函数 是实例方法
*/	
	...
	public Testtwo(String name,int age)		
	{
		this.name = name;
		this.age = age;
		System.out.println("----这是有参数的构造函数----" + this.name + "，" + this.age);
	}
